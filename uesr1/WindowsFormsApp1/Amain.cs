...
namespace WindowsFormsApp1
{
    public partial class Amain : Form
    {
        ...
        public Amain()
        {
            InitializeComponent();
            CreateTreeNode();
            LoadForm(MainTabControl2, "出库--入库", typeof(Forms.Form5));
        }
        private void CreateTreeNode()
        {
            int pageIndex = 1000;
            //一级节点
            TreeNode parent2 = Aside.CreateNode("老炮儿", 62094, 22, pageIndex);
        ...
        private void uiNavMenu1_MenuItemClick(TreeNode node, Sunny.UI.NavMenuItem item, int pageIndex)
        {
            if (item != null)
            {
                ...
                switch (menuText)
                {
                    ...
                    case "出库&入库":
                        LoadForm(MainTabControl2, menuText, typeof(Forms.Form5));
                        break;
                    case "物资管理":
